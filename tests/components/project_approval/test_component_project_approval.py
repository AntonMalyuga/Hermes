...
def test_component_project_approval_all_approved(driver, project_id):
    Project(driver).open_order(project_id)
...
def test_component_project_approval_all_decline(driver, project_id):
    Project(driver).open_order(project_id)
    Project(driver).check_current_stage(
        stage_name='Согласование ТР с КБ по всему проекту (Уточнения намерений клиента)', second_do_reload=30)
    ComponentCloseStage(driver).close_stage(pass_name='Положительно', next_stage='Ожидание подтверждения ТР в БТИ')
...
def test_component_project_approval_in_part_approved(driver, project_id):
    Project(driver).open_order(project_id)
    Project(driver).check_current_stage(
        stage_name='Согласование ТР с КБ по всему проекту (Уточнения намерений клиента)', second_do_reload=30)
    ComponentCloseStage(driver).close_stage(pass_name='Положительно', next_stage='Ожидание подтверждения ТР в БТИ')
