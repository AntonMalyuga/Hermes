from locator import Locator
from page import Page
import testit


class ClaimsActivitiesReport(Page):
    name = 'Отчёт для выявления фактов просрочки по стройке со стороны контрагентов'
    path = 'report/claims_activities_report'

    _is_open_report = '//button[contains(., "Показать на экране")]'
    _LOCATOR_H2_NAME_REPORT = '//h2'

    @staticmethod
    def is_open_report() -> bool:
        with testit.step(
                f'Проверить открытие отчета "{ClaimsActivitiesReport.name}" по адресу "{ClaimsActivitiesReport.path}"'):
            return Locator(ClaimsActivitiesReport._is_open_report).is_on_page()

    @staticmethod
    def get_name_report() -> str:
        name = Locator(ClaimsActivitiesReport._LOCATOR_H2_NAME_REPORT).text
        with testit.step(f'Получить имя отчёта в интерфейсе: {name}'):
            return name
